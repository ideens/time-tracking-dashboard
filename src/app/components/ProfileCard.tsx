...
const handleSelectedTime = (period: "daily" | "weekly" | "monthly") => {
  ...
  return (
    <section className='sm:w-full lg:w-64 sm:h-[203px] lg:h-[518px] lg:mr-[15px] xl:mr-[25px] sm:max-lg:mb-6 relative'>
      ...
      <p className='font-rubik-light text-purple-text sm:pt-1 lg:pt-10 text-[15px]'>Report for</p>
      <h1 className='font-rubik-light sm:text-[24px]/[28px] lg:text-[40px]/[52px] text-white pt-1'>Jeremy Robson</h1>
      </div>
      </div>
      <div className='bg-blue-primary sm:w-full lg:w-64 h-full px-8 py-6 rounded-[15px] flex flex-col justify-end'>
        <div className='flex sm:flex-row sm:max-lg:justify-between lg:flex-col text-purple-heading gap-4'>
          ...
          className={`hover:text-white text-${handleSelectedTime(TIME_PERIOD.DAY)} custom-transition`}
          >
            Daily
          </li>
          <li 
          onClick={() => setTimeSetting(TIME_PERIOD.WEEK)}
          className={`hover:text-white text-${handleSelectedTime(TIME_PERIOD.WEEK)} custom-transition`}
          >
            Weekly
            </li>
          <li 
          onClick={() => setTimeSetting(TIME_PERIOD.MONTH)}
          className={`hover:text-white text-${handleSelectedTime(TIME_PERIOD.MONTH)} custom-transition`}
          >
            Monthly
            </li>
        </div>
